...
class MemoryStore:
    ...
    async def recall(self, goal: str, limit: int = 3) -> list[str]:
        ...
        Relevance is scored by word-overlap with the goal text so that memories
        from similar runs surface first.  Falls back to the most recent memories
        when there is no vocabulary overlap.
        """
        try:
            async with session_scope() as session:
                result = await session.execute(
                    select(Memory).order_by(Memory.created_at.desc()).limit(limit * 5)
                )
